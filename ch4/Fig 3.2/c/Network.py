...
class QIFNetwork() :
    ...
    def __init__(self, T, N, tau_m, tau_d, tau_s1, tau_s2, I, Js, Jc, g, eta_mean, delta, VP, VR, STEP, v0, s0, r0):
    ...
    def make_step_pop1(self, t, v, r, s, v_avg, r_avg, spike_times):
        ...
        v_next[v >= self.VP] = self.VR

        spike_times_next[(spike_times != 0) & (t > spike_times + self.tau_s1)] = 0

        r_avg_next = np.mean((spike_times != 0) & (t <= spike_times + self.tau_s1))
        r_avg_next = r_avg_next/self.tau_s1
    ...
    def make_step_pop2(self, t, v, r, s, v_avg, r_avg, spike_times):
        ...
        spike_times_next[(spike_times != 0) & (t > spike_times + self.tau_s2)] = 0

        r_avg_next = np.mean((spike_times != 0) & (t <= spike_times + self.tau_s2))
        r_avg_next = r_avg_next/self.tau_s2
        v_avg_next = np.mean(v_next)
    ...
    def plot_system(self, analytical=True) :
        ...
        ax[0,0].plot(times, self.input, color='black')
        ax[0,1].plot(times, self.input, color='black')
        ...
        ax[1,0].plot(times_v1, self.v1, c='k')
        ax[1,1].plot(times_v1, self.v2, c='k')
        ...
        ax[2,0].plot(times1, self.r1, c='k')
        ax[2,1].plot(times2, self.r2, c='k')
        ...
        ax[3,0].plot(times1, self.s1, c='k')
        ax[3,1].plot(times2, self.s2, c='k')
        ...
        ax[4,0].scatter(self.times_raster1, self.raster1, s=0.5, alpha=0.5, c='k')
        ax[4,1].scatter(self.times_raster2, self.raster2, s=0.5, alpha=0.5, c='k')
        ...
        if analytical :
            ...
            ax[1,0].plot([self.STEP*k for k in range(len(v_sol1))], v_sol1, c='r', label='analytical')
            ax[2,0].plot([self.STEP*k for k in range(len(r_sol1))], r_sol1, c='r')
            ax[3,0].plot([self.STEP*k for k in range(len(s_sol1))], s_sol1, c='r')

            ax[1,1].plot([self.STEP*k for k in range(len(v_sol2))], v_sol2, c='r', label='analytical')
            ax[2,1].plot([self.STEP*k for k in range(len(r_sol2))], r_sol2, c='r')
            ax[3,1].plot([self.STEP*k for k in range(len(s_sol2))], s_sol2, c='r')
