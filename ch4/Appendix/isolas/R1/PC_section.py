...
my_file = Path('periods.txt')

if not my_file.is_file():
    for J2 in np.arange(-3.1, -2.9, 0.001):
        # Chemical coupling strength in population 1, population 2
        J = [-2.5, J2]

        v1, v2, r1, r2, s1, s2 = euler(r0, v0, s0, g, J)
        #r0, v0, s0 = [r1[-1], r2[-1]], [v1[-1], v2[-1]], [s1[-1], s2[-1]]

        r1 = r1[-int(800/STEP):]
        peaks, _ = find_peaks(r1)
        peaks_neg, _ = find_peaks(-r1)

        r1 = np.round(r1, 5)

        r_values_p = np.unique(r1[peaks])
        r_values_m = np.unique(r1[peaks_neg])

        if len(r1[peaks]) == 0 and len(r1[peaks_neg]) == 0:
            plt.scatter(J2, r1[-1], c='k', s=5)
    ...
    with open('periods.txt') as f:
        for line in f:
            ...
            if line[0] >= -3.076 and line [0] <= -3.025: 
                plt.scatter(line[0]*np.ones(len(line[1:])), line[1:], c='r', s=8, alpha=.5, zorder=10)
            

    with open('periods_cont.txt') as f:
        for line in f:
            line = line.replace('\n', '')
            line = line.split(' ')
            line = list(filter(None, line))
