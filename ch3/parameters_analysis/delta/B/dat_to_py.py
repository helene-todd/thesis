...
for filename in args.files :
    ...
    for k in range(len(g)) :
        if stability[k] == 1 :
            plt.plot(g[k], J[k], color=colors[k], linewidth=2, linestyle=s[0], label='focus node')
        if stability[k] == 2 :
            plt.plot(g[k], J[k], color=colors[k], linewidth=2, linestyle=s[1], label='limit cycle')
        if stability[k] == 3 :
            plt.plot(g[k], J[k], color=colors[k], linewidth=2, linestyle=s[1], label='limit cycle')
    col += 1
