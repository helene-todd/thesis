...
for filename in args.files :
    ...
    for k in range(len(g)) :
        if stability[k] == 1 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[0], label=f'$I={eta[col]}$')
        if stability[k] == 2 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[1], label=f'$I={eta[col]}$')
        if stability[k] == 3 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[2], label=f'$I={eta[col]}$')
    col += 1
