...
for filename in args.files :
    ...
    for k in range(len(g)) :
        if stability[k] == 1 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[0], label=f'$b={eta[col]}$', zorder=order[col])
        if stability[k] == 2 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[1], label=f'$b={eta[col]}$', zorder=order[col])
        if stability[k] == 3 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[1], label=f'$b={eta[col]}$', zorder=order[col])
    col += 1
...
plt.legend(by_label.values(), by_label.keys(), loc='upper left') # bbox_to_anchor=(1, 0.95))

#plt.title('Different spikelet attenuations bifurcations', size=14) #, pad=20)
plt.tight_layout()
plt.savefig('spikelet_attenuations.png', dpi=600)
plt.show()
