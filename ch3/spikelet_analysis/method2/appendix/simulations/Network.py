...
class QIFNetwork() :
    ...
    def QIF(self, t, v, r, s, v_avg, r_avg):
        dv = (v ** 2 + self.tau_m*self.J*s + self.g*(self.f(v_avg)-v) + self.eta + self.I[int(t/self.STEP)-1]) / self.tau_m
    ...
    def euler(self):
        ...
        for i in range(1, int(self.T/self.STEP)):
            ...
            v.append(v[i-1] + self.STEP*(v[i-1]**2 + self.eta_mean + self.J*self.tau_m*s[i-1] + self.I[i-1] - (math.log(self.a)**2 + math.pi**2)*(self.tau_m*r[i-1])**2 + self.delta*math.log(self.a)/math.pi + self.g*(self.f(v[i-1])-v[i-1])) /self.tau_m)
    ...
    def plot_system(self, analytical=True) :
        ...
        ax[4].set_yticks([0, self.selected_neurons], [r'$0$', r'$1e3$'])
        ax[4].set_xlabel('time')
        ax[4].set_xlim(100, self.T)
        ax[4].set_xticks([100, 120, 140, 160, 180, 200], [0, 20, 40, 60, 80, 100])
