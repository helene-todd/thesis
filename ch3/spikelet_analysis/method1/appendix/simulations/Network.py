...
class QIFNetwork() :
    ...
    def plot_system(self, analytical=True) :
        ...
        ax[0].plot(times_v, self.v, c='k', label='simulation')
        ...
        ax[2].plot(times, self.s, c='k')
        ax[2].set_ylabel(r'$s(t)$')

        # Plotting raster plot
        ax[3].set_ylabel('neuron #')
        ax[3].scatter(self.times_raster, self.raster, s=0.9, alpha=0.25, c='k')
        ax[3].set_ylim(0, self.selected_neurons)
        ax[3].set_yticks([0, 1000], [r'$0$', r'$1e3$'])
        ...
        if analytical :
            ...
            ax[0].plot([self.STEP*k for k in range(len(v_sol))], v_sol, c='r', label='analytical')
            ax[1].plot([self.STEP*k for k in range(len(r_sol))], r_sol, c='r')
            ax[2].plot([self.STEP*k for k in range(len(s_sol))], s_sol, c='r')
