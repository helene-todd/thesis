...
class QIFNetwork() :
    ...
    def __init__(self, T, N, tau_m, tau_s, J, g, eta_mean, I, delta, VP, VR, REFRACT_TIME, STEP, v0, r0):
    ...
    def QIF(self, t, v, v_avg, r_avg):
        dv = (v ** 2 + self.tau_m*self.J*r_avg + self.g*(v_avg-v) + self.eta + self.I[int(t/self.STEP)-1]) / self.tau_m
        return dv
    ...
    def euler(self):
        ...
        for i in range(1, int(self.T/self.STEP)):
            ...
            v.append(v[i-1] + self.STEP*(v[i-1]**2 + self.eta_mean + self.I[i-1] + self.J*self.tau_m*r[i-1] -  (math.pi**2)*(self.tau_m*r[i-1])**2)/self.tau_m)
