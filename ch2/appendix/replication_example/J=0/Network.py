...
class QIFNetwork() :
    ...
    def euler(self):
        ...
        for i in range(1, int(self.T/self.STEP)):
            ...
            v.append(v[i-1] + self.STEP*(v[i-1]**2 + self.eta_mean + self.J*self.tau_m*r[i-1] - (math.log(self.a)**2 + math.pi**2)*(self.tau_m*r[i-1])**2 + self.delta*math.log(self.a)/math.pi )/self.tau_m)
    ...
    def plot_system(self, analytical=True) :
        ...
        ax[2].set_yticks([0, 10000], [r'$0$', r'$1e3$'])
